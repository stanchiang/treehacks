...
def search(keyword):
    FILTER=keyword
    FILTER=FILTER.lower()
    directory= str(os.path.realpath(__file__)).replace('/clustering/clustering_by_words_frequency.py','').replace('stickerc','sticker')
    print directory
    ...
    directory=directory.replace('/sticker','')
    f=open(directory+ '/static/sticker/clustering/'+'data1.json',"w")
    ...
    f=open(directory+ '/static/sticker/clustering/'+'data2.json',"w")
    ...
    for i in range(n):
        for j in range(n):
            if j<i:
                if NETWORK3['nodes'][i]['group']==NETWORK3['nodes'][j]['group'] and NETWORK2['nodes'][j]['group']!='NA' :
                    NETWORK3['links'].append({"source":i,"target":j,"value":NETWORK3['nodes'][i]['group']})


    pprint.pprint(NETWORK3)  

    f=open(directory+ '/static/sticker/clustering/'+'data3.json',"w")
    ...
    print directory
    f=open(directory+ '/static/sticker/clustering/'+'data4.json',"w")
